...
local function do_configure(self,device)
  ...
  if device:get_field("moisture_Water_Sensor") == nil then  device:set_field("moisture_Water_Sensor", 0, {persist = false}) end
...
local pressure_value_attr_handler = function (driver, device, value, zb_rx)
  ...
  if math.abs(value.value - last_value) >= device.preferences.pressChangeRep * 10 or (os.time() - last_value_time) + 20 >= (device.preferences.pressMaxTime * 60) then
    ...
    device:set_field("last_value", value.value, {persist = false})
...
local function humidity_attr_handler(driver, device, value, zb_rx)
  ...
  if device:get_field("last_humidity") == nil then device:set_field("last_humidity", 0, {persist = false}) end
  local last_humidity = device:get_field("last_humidity")
  if device:get_field("last_humidity_time") == nil then device:set_field("last_humidity_time", (os.time() - (device.preferences.humiMaxTime * 60)) , {persist = false}) end
  ...
  if math.abs(value.value - last_humidity) >= device.preferences.humiChangeRep * 100 or (os.time() - last_humidity_time) + 20 >= (device.preferences.humiMaxTime * 60) then  
    ...
    -- save emitted pressure value
    device:set_field("last_humidity", value.value, {persist = false})
    device:set_field("last_humidity_time", os.time(), {persist = false})
  end
end
...
local function analog_adc_Input_Handler(self,device,value)
 ...
 if device:get_field("analog_Input_Enable") == "Enabled" and last_vdd > 0 then
  ...
  if device:get_field("last_water_state") == nil then device:set_field("last_water_state", waterSensorState, {persist = false}) end
  if device:get_field("last_volts") == nil then
    local last_volts = device:get_latest_state("main", capabilities.voltageMeasurement.ID, capabilities.voltageMeasurement.voltage.NAME)
    if last_volts == nil then last_volts = 0 end
    device:set_field("last_volts", last_volts, {persist = false}) 
  end
  ...
  print (">>> Volts- last_volts =", math.abs(volts - device:get_field("last_volts")))
   ...
   if math.abs(volts - device:get_field("last_volts")) >= device.preferences.voltsRepChange / 10 or device:get_field("last_water_state") ~= waterSensorState then
    ...
    device:set_field("last_volts", volts, {persist = false})
    device:set_field("last_water_state", waterSensorState, {persist = false})
    ...
    device:set_field("moisture_Water_Sensor", moisture_Sensor, {persist = false})
    device:emit_event(moisture_Water_Sensor.moistureWaterSensor(moisture_Sensor))
   end

  elseif device:get_field("analog_Input_Enable") == "Disabled" then

   device:emit_event_for_endpoint("main", capabilities.waterSensor.water.dry())
   device:emit_event(capabilities.voltageMeasurement.voltage(0))
   device:set_field("moisture_Water_Sensor", 0, {persist = false})
...
local function binary_Input_Handler(self,device,value)
 ...
 if device:get_field("binary_Input_Enable") == "Enabled" then
  ...
  if device:get_field("binary_text") == nil then device:set_field("binary_text", " ", {persist = false}) end
  if binary_text ~= device:get_field("binary_text") then
    device:set_field("binary_text", binary_text, {persist = false})
...
local function analog_Input_Enable_handler(self,device,command)
  ...
  if command.args.value == "Disabled" then
    device:emit_event(capabilities.voltageMeasurement.voltage(0))
    device:set_field("last_volts", 0, {persist = false})
    device:emit_event_for_endpoint("main", capabilities.waterSensor.water.dry())
    device:set_field("moisture_Water_Sensor", 0, {persist = false})
    device:emit_event(moisture_Water_Sensor.moistureWaterSensor(0))
