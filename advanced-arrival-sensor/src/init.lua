...
local function presence_timer_handler(self, device)
  ...
  for timer in pairs(device.thread.timers) do
    ...
    device:set_field("presence_timer_status", "OFF", {persist = true})
    device.thread:cancel_timer(timer)
  end

   print("<<<<< Timer function >>>>>")
    device:set_field("presence_timer_status", "ON", {persist = true})
...
local function do_init(self, device)
  ...
  if device:get_field("last_presence_check") == nil then device:set_field("last_presence_check", os.time(), {persist = true}) end
  if device:get_field("presence_timer_status") == nil then device:set_field("presence_timer_status", "OFF", {persist = true}) end
  if device:get_field("power_source") == nil then device:set_field("power_source", "battery", {persist = true}) end
  if device:get_field("last_battery_value") == nil then device:set_field("last_battery_value", 0, {persist = true}) end
...
local function temp_attr_handler(self, device, tempvalue, zb_rx)
  ...
  ---- emit presence event
  device:emit_event(capabilities.presenceSensor.presence("present"))
  device:set_field("last_presence_check", os.time(), {persist = true})
...
local function binary_Input_StatusFlags_Handler(self, device, value)
  ...
  if status_Falgs == 0 or status_Falgs == 8 then
    if device:get_field("motion_Sensor_Enable") == "Enabled" then device:emit_event(capabilities.motionSensor.motion.inactive()) end
    ...
    device:set_field("power_source", "battery", {persist = true})
  ...
  elseif status_Falgs == 1 then
    ...
    device:set_field("power_source", "dc", {persist = true})
...
device:emit_event(capabilities.voltageMeasurement.voltage(value.value / 10))
 ...
 device:set_field("last_battery_value", battery_pct, {persist = true})
--end
end
  ...
  zigbee_handlers = {
    attr = {
      ...
      [PowerConfiguration.ID] = {
        ...
        --[PowerConfiguration.ID] = {
          --[PowerConfiguration.attributes.BatteryPercentageRemaining.ID] = BatteryPercentageRemaining_Handler
        --},
      },
  },
